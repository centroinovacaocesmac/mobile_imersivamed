...
import { fetchQuestionsByTopic } from "@services/assessmentFirebase.services";
...
export default function AssessmentScreen(){
    ...
    const { title, questions, selectedOptions, selectOption, setAssessment} = useAssessmentStore();
    ...
    useEffect(() => {
        const load = async () => {
            if(typeof topic === "string"){
                ...
                setAssessment(topic, mappedQuestions);
            }
            setLoading(false);
        }
    ...
    const handleNext = () => {
        if(current + 1 < questions.length){
            ...
            const correctAnswers = questions.filter((q, index) => selectedOptions[index] === q.correct).length;
            const incorrectAnswers = questions.length - correctAnswers;

            useAssessmentStore.getState().setResults(correctAnswers, incorrectAnswers);
            
            router.push("/(notabsone)/Screens/ResultAssessment")
        }
    };
    ...
    if (!questions.length) {
        return (
            <SafeAreaView className="flex-1 justify-center items-center bg-white px-6">
                ...
                <Button name="Voltar" onPress={() => router.back()} nameClass="bg-mainColor mt-4 px-4 py-4 rounded-[10px]" nameClassText="color-white text-lg text-center" iconClass="hidden"/>
            </SafeAreaView>
        );
    }
                ...
                <View className="flex gap-10 pb-4">
                    ...
                    <View className="flex flex-row gap-4 w-full">
                        ...
                        <Button
                            ...
                            } flex-1 flex-row justify-center rounded-[10px] flex-1 py-4`}
                            nameClassText="color-white text-xl text-center"
                            iconClass="hidden"
                            onPress={handleNext}
