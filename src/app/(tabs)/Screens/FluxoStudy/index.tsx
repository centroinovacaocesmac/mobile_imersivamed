...
export default function CapillariesStudy(){
    ...
    useEffect(() => {
        const loadContents = async () => {
            const data = await fetchContentsByTopic("Circulação sistêmica");
            setContents(data);
        };
        loadContents();
    }, []);
            ...
            <ScrollView style={{ marginTop: statusBarHeight + 8 }} showsVerticalScrollIndicator={false} className="flex-1 bg-white px-4">
                <View className="flex gap-10 pb-4">
                    <View className="flex flex-row h-8">
                        ...
                        <Title nameClassOne="flex-1 items-center" nameClassTextOne="color-blackColor1 text-2xl font-medium" textOne="Circulação sistêmica"/>
                    </View>
                    <View className="gap-2">
                        <TouchableOpacity className="border-[0.4px] border-mainColorVariant2 rounded-[10px] h-52 flex justify-center items-center" onPress={() => router.push({pathname: "/(notabstwo)/Screens/View3DModel", params: {id: "sistemica"}})}>
                            <Image source={SystemicImage} className="w-40 h-40"/>
                        </TouchableOpacity>
                    ...
                    <View>
                        <Button name="Avaliar aprendizado" nameClass="bg-mainColor flex flex-row justify-center rounded-[10px] p-4" nameClassText="color-white text-xl" iconClass="hidden" onPress={() => router.push("/(notabstwo)/Screens/AssessmentScreen?topic=Circulação sistêmica")}/>
                    </View>
                </View>
            </ScrollView>
        </SafeAreaView>
