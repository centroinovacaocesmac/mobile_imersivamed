...
export default function CapillariesStudy(){
    ...
    useEffect(() => {
        const loadContents = async () => {
            const data = await fetchContentsByTopic("Coração");
            setContents(data);
        };
        loadContents();
    }, []);
            ...
            <ScrollView style={{ marginTop: statusBarHeight + 8 }} showsVerticalScrollIndicator={false} className="flex-1 bg-white px-4">
                <View className="flex gap-10 pb-4">
                    <View className="flex flex-row h-8">
                        ...
                        <Title nameClassOne="flex-1 items-center" nameClassTextOne="color-blackColor1 text-2xl font-medium" textOne="Coração"/>
                    </View>
                    ...
                    <View>
                        <Button name="Avaliar aprendizado" nameClass="bg-mainColor flex flex-row justify-center rounded-[10px] p-4" nameClassText="color-white text-xl" iconClass="hidden" onPress={() => router.push("/(notabstwo)/Screens/AssessmentScreen?topic=Coração")}/>
                    </View>
                </View>
            </ScrollView>
        </SafeAreaView>
